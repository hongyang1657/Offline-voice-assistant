...
public class MediaReportModel implements IMediaReportPresenter {

    private MediaReportPresenter mediaReportPresenter;

    public MediaReportModel(MediaReportPresenter mediaReportPresenter) {
        this.mediaReportPresenter = mediaReportPresenter;
    }
    ...
    public void mediaReport(String url, int position, String musicPlayerStatus) {
        ...
        params.put("method","player/report/status");
        params.put("api_key", apikey);
        params.put("timestamp", timeStamp);
        params.put("version", HttpConstant.API_VERSION);

        LinkedHashMap<String, Object> map = new LinkedHashMap<>();
        map.put("method", "player/report/status");
        ...
        value.put("device_status",musicPlayerStatus);
        map.put("value",value);
        Gson gson = new Gson();
        params.put("http_body", gson.toJson(map));
        String sign = SignAndEncrypt.signRequest(params, apiSecret);
        ApiManager.MediaPlayerService.mediaReport(apikey,timeStamp,sign,HttpConstant.API_VERSION,"player/report/status",map)
                ...
                .subscribe(new Subscriber<Status>() {
                    ...
                    public void onError(Throwable e) {
                        L.i("媒体播放器向后台同步播放状态的结果e:"+e.toString());
                    }

                    @Override
                    public void onNext(Status responseBody) {
                        mediaReportPresenter.onSucess(responseBody);
